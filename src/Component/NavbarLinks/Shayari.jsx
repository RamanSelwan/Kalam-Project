...
const Shayari = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shayariPosts = postResult.filter((post) => post.type === 'Shayari');
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Shayari</h1>
      {error && <div className="text-red-500 mb-4">{error}</div>}
      {posts.length === 0 ? (
        <div className="text-center text-gray-600">No Shayari posts available.</div>
      ) : (
